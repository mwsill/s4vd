stabpath <- function(res,k){
	...
	if(class(res@info$Rspath[[k]])=="numeric"){
		...
		plot(res@info$Rspath[[k]],ylim=c(0,1),col=cols,ylab=paste("selection probability"),xlab="rows")
		...
		plot(res@info$Cspath[[k]],ylim=c(0,1),col=cols,ylab=paste("selection probability"),xlab="cols")
