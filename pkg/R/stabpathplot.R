stabpath <- function(res,number){
	...
	if(res@Parameters$savepath&!res@Parameters$pointwise){
		...
		colsv[redv] <- "red"
		colsu[redu] <- "red"
		matplot(t(vc[[8]][,!is.na(colSums(vc[[8]]))]),type="l",col=colsv,lty=1,ylab=expression(paste(hat(Pi)[v]))
		...
		matplot(t(uc[[8]][,!is.na(colSums(uc[[8]]))]),type="l",col=colsu,lty=1,ylab=expression(paste(hat(Pi)[u]))
